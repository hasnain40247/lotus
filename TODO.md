...
- [x] Turn timer + context gauge in TUI — spinner row shows live `{Ns} · {N tok}` for the current turn (elapsed frozen at completion, resets on next idle→busy); agent footer's right side shows `{pct}% ({total})` context usage, computed from the last assistant message's `tokens` vs. the model's `limit.context` (fetched from `sdk.client.provider.list`)
