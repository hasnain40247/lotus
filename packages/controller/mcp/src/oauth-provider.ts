 ...
 *
 * Ported from packages/neko/src/mcp/oauth-provider.ts.
 */
...
export class McpOAuthProvider implements OAuthClientProvider {
  constructor(
    ...
    protected auth: McpAuthController.Interface,
  ) {}
  ...
  async state(): Promise<string> {
    ...
    // startAuth() hasn't pre-saved one (e.g. during automatic auth on first connect).
  ...
  async invalidateCredentials(type: "all" | "client" | "tokens"): Promise<void> {
    ...
    switch (type) {
      case "all":
        ...
        break
      case "client": {
        const { clientInfo: _ci, ...rest } = entry
        await Effect.runPromise(this.auth.set(this.mcpName, rest))
        break
      }
      case "tokens": {
...
/**
 * Variant of McpOAuthProvider used during the startAuth flow:
 * client info and tokens are buffered in-memory until commit() is called,
 * avoiding premature writes to the auth store while the OAuth dance is in progress.
 */
export class McpOAuthPendingProvider extends McpOAuthProvider {
  ...
  /** Flush pending tokens/client-info to the auth store. */
...
export function parseRedirectUri(redirectUri?: string): { port: number; path: string } {
  ...
  try {
    ...
    const parsedPath = url.pathname || OAUTH_CALLBACK_PATH
    return { port, path: parsedPath }
  } catch {
    return { port: OAUTH_CALLBACK_PORT, path: OAUTH_CALLBACK_PATH }
  }
}
