import type { ExperimentalWorkspaceAdapterListResponse, Workspace } from "@gco/sdk/v2"
...
export async function warpWorkspaceSession(input: {
  ...
  if (!result?.data) {
    if (result?.error && typeof result.error === "object" && result.error !== null && "name" in result.error && (result.error as any).name === "VcsApplyError") {
