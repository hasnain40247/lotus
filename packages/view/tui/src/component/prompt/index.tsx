...
import { registerNekoSpinner } from "../register-spinner"
...
import type { AssistantMessage, FilePart, UserMessage } from "@gco/sdk/v2"
...
import { NEKO_BASE_MODE, useBindings, useCommandShortcut, useLeaderActive, useNekoKeymap } from "../../keymap"
...
function ThinkingLabel(props: { fg: RGBA; shine: RGBA }) {
  const [wordIndex, setWordIndex] = createSignal(Math.floor(Math.random() * THINKING_WORDS.length))
  const [shinePos, setShinePos] = createSignal(-SHINE_WIDTH)
  ...
  const timer = setInterval(() => {
    setShinePos((pos) => {
      const next = pos + 1
      if (next > word().length + SHINE_WIDTH) {
  ...
  const start = () => Math.max(0, shinePos())
  const end = () => Math.min(word().length, shinePos() + SHINE_WIDTH)
  const prefix = () => word().slice(0, start())
  const shine = () => word().slice(start(), end())
  const suffix = () => word().slice(end())

  return (
    <text>
      <span style={{ fg: props.fg }}>{prefix()}</span>
      <span style={{ fg: props.shine }}>{shine()}</span>
      <span style={{ fg: props.fg }}>{suffix()}</span>
    </text>
  )
}
...
export function Prompt(props: PromptProps) {
  ...
  createEffect(() => {
    if (!input || input.isDestroyed) return
    if (props.disabled) input.cursorColor = theme.backgroundMenu
  ...
  const promptCommands = createMemo(() =>
    [
      ...
      },
      {
        ...
        name: "workspace.set",
        category: "Session",
        enabled: Flag.NEKO_EXPERIMENTAL_WORKSPACES,
        slashName: "warp",
        run: () => {
          workspace.open()
        },
        ...
        >
          ...
          >
            <textarea
              ...
              cursorColor={props.disabled ? theme.backgroundMenu : theme.text}
              syntaxStyle={syntax()}
            />
                ...
                <box flexShrink={0} flexDirection="row" gap={1}>
                  <box marginLeft={1}>
                    <Show when={kv.get("animations_enabled", true)} fallback={<text fg={theme.textMuted}>·</text>}>
                      <spinner frames={["·", "·", "·", "✦"]} interval={200} color={theme.textMuted} />
                    </Show>
                  </box>
                  <ThinkingLabel fg={theme.textMuted} shine={theme.text} />
