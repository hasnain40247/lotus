import type { TuiPluginApi } from "../../plugin-types"
...
function configShortcut(api: TuiPluginApi, command: string): TipShortcut {
  return () =>
    (api.tuiConfig.keybinds?.get(command) ?? [])
      .map((binding) => api.keys.formatSequence(Array.from((api.keymap.parseKeySequence as ((k: unknown) => unknown[]) | undefined)?.(( binding as any).key) ?? [])))
      .filter(Boolean)
      .join(", ")
}
...
export function Tips(props: { api: TuiPluginApi; connected?: boolean }) {
  ...
  const shortcuts: Shortcuts = {
    ...
    statusView: useCommandShortcut("lotus-code.status"),
...
const TIPS: Tip[] = [
  ...
  "Run {highlight}/share{/highlight} to create a public link to your conversation at lotus-code.ai",
  ...
  "Type {highlight}/{/highlight} to see all available commands",
  ...
  "Create {highlight}lotus-code.json{/highlight} for server settings and {highlight}tui.json{/highlight} for TUI settings",
  "Place TUI settings in {highlight}~/.config/lotus-code/tui.json{/highlight} for global config",
  "Add {highlight}$schema{/highlight} to your config for autocomplete in your editor",
  "Configure {highlight}model{/highlight} in config to set your default model",
  "Override any keybind in {highlight}tui.json{/highlight} via the {highlight}keybinds{/highlight} section",
  "Set any keybind to {highlight}none{/highlight} to disable it completely",
  "Configure local or remote MCP servers in the {highlight}mcp{/highlight} config section",
  "Add {highlight}.md{/highlight} files to {highlight}.lotus-code/commands/{/highlight} to define reusable custom prompts",
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks in commands to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.lotus-code/agents/{/highlight} for specialized AI personas",
  ...
  "Create {highlight}.ts{/highlight} files in {highlight}.lotus-code/tools/{/highlight} to define new LLM tools",
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.lotus-code/plugins/{/highlight} for event hooks",
  "Use plugins to send OS notifications when sessions complete",
  "Create a plugin to prevent Lotus Code from reading sensitive files",
  "Use {highlight}lotus-code run{/highlight} for non-interactive scripting",
  "Use {highlight}lotus-code --continue{/highlight} to resume the last session",
  "Use {highlight}lotus-code run -f file.ts{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}lotus-code serve{/highlight} for headless API access to Lotus Code",
  "Use {highlight}lotus-code run --attach{/highlight} to connect to a running server",
  "Run {highlight}lotus-code upgrade{/highlight} to update to the latest version",
  "Run {highlight}lotus-code auth list{/highlight} to see all configured providers",
  "Run {highlight}lotus-code agent create{/highlight} for guided agent creation",
  "Use {highlight}/lotus-code{/highlight} in GitHub issues/PRs to trigger AI actions",
  "Run {highlight}lotus-code github install{/highlight} to set up the GitHub workflow",
  "Comment {highlight}/lotus-code fix this{/highlight} on issues to auto-create PRs",
  "Comment {highlight}/oc{/highlight} on PR code lines for targeted code reviews",
  'Use {highlight}"theme": "system"{/highlight} to match your terminal\'s colors',
  "Create JSON theme files in {highlight}.lotus-code/themes/{/highlight} directory",
  ...
  "Run {highlight}lotus-code debug config{/highlight} to troubleshoot configuration",
  ...
  "Toggle username display in chat via {highlight}/config{/highlight}",
  "Run {highlight}docker run -it --rm ghcr.io/anomalyco/lotus-code{/highlight} for containerized use",
  "Use {highlight}/connect{/highlight} with Lotus Code Zen for curated, tested models",
