...
export function Tips(props: { api: TuiPluginApi; connected?: boolean }) {
  ...
  const shortcuts: Shortcuts = {
    ...
    statusView: useCommandShortcut("neko.status"),
...
const TIPS: Tip[] = [
  ...
  "Run {highlight}/share{/highlight} to create a public link to your conversation at neko.ai",
  ...
  "Create {highlight}neko.json{/highlight} for server settings and {highlight}tui.json{/highlight} for TUI settings",
  "Place TUI settings in {highlight}~/.config/neko/tui.json{/highlight} for global config",
  "Add {highlight}$schema{/highlight} to your config for autocomplete in your editor",
  "Configure {highlight}model{/highlight} in config to set your default model",
  "Override any keybind in {highlight}tui.json{/highlight} via the {highlight}keybinds{/highlight} section",
  "Set any keybind to {highlight}none{/highlight} to disable it completely",
  "Configure local or remote MCP servers in the {highlight}mcp{/highlight} config section",
  "Add {highlight}.md{/highlight} files to {highlight}.neko/commands/{/highlight} to define reusable custom prompts",
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks in commands to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.neko/agents/{/highlight} for specialized AI personas",
  ...
  "Create {highlight}.ts{/highlight} files in {highlight}.neko/tools/{/highlight} to define new LLM tools",
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.neko/plugins/{/highlight} for event hooks",
  "Use plugins to send OS notifications when sessions complete",
  "Create a plugin to prevent Neko Code from reading sensitive files",
  "Use {highlight}neko run{/highlight} for non-interactive scripting",
  "Use {highlight}neko --continue{/highlight} to resume the last session",
  "Use {highlight}neko run -f file.ts{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}neko serve{/highlight} for headless API access to Neko Code",
  "Use {highlight}neko run --attach{/highlight} to connect to a running server",
  "Run {highlight}neko upgrade{/highlight} to update to the latest version",
  "Run {highlight}neko auth list{/highlight} to see all configured providers",
  "Run {highlight}neko agent create{/highlight} for guided agent creation",
  "Use {highlight}/neko{/highlight} in GitHub issues/PRs to trigger AI actions",
  "Run {highlight}neko github install{/highlight} to set up the GitHub workflow",
  "Comment {highlight}/neko fix this{/highlight} on issues to auto-create PRs",
  "Comment {highlight}/oc{/highlight} on PR code lines for targeted code reviews",
  'Use {highlight}"theme": "system"{/highlight} to match your terminal\'s colors',
  "Create JSON theme files in {highlight}.neko/themes/{/highlight} directory",
  ...
  "Run {highlight}neko debug config{/highlight} to troubleshoot configuration",
  ...
  "Run {highlight}docker run -it --rm ghcr.io/anomalyco/neko{/highlight} for containerized use",
  "Use {highlight}/connect{/highlight} with Neko Code Zen for curated, tested models",
