...
import type { TuiPlugin, TuiPluginApi, TuiRouteCurrent } from "../../plugin-types"
import type { SnapshotFileDiff, VcsFileDiff } from "@gco/sdk/v2"
import {
  TextAttributes,
  type BorderSides,
  type BoxRenderable,
...
function DiffViewer(props: { api: TuiPluginApi }) {
  ...
  const [diff] = createResource(diffInput, async (input) => {
    if (input.mode === "last-turn") {
      ...
      return normalizeDiffs((result.data ?? []) as any)
    }
  ...
  const patchScrollAcceleration = createMemo(() => getScrollAcceleration(props.api.tuiConfig as any))
